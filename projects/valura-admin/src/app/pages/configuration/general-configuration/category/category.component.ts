...
export class CategoryComponent {
  ...
  dataCategoryFullAcess: boolean = false;
  ...
  ngOnInit() {
    this.getCategory(this.searchText, this.sortByField, this.sortDirection);
    this.setUserPermissions();
  }

  setUserPermissions() {
    this.dataCategoryFullAcess = this.rolePermissionService.fullAccessConfiguration;
  }
  ...
  async getCategory(
    ...
    try {
      const category = await this.pdElementService.getCategory();
      if (category && category.length) {
        this.originalElements = category;
        this.applySearchAndSort();
      } else {
  ...
  async refreshTableData() {
    await this.getCategory(this.searchText, '', '');
  ...
  async onDeleteClick(element: any) {
    const dialogRef = this.dialog.open(PopupDialogComponent, {
      ...
      data: {
        ...
        content: 'Are you sure you want to delete this Data Category?',
    ...
    dialogRef.afterClosed().subscribe(async (confirmed: boolean) => {
      ...
      try {
        const res = await this.configApiHelperService.onDeletePdCategory(
          element.id
        );
        this.pdElementService.deletPdCategory(element.id);
        if (res?.success) {
        ...
        console.error('Error deleting PD element Category:', error);
      }
    });
  }
  ...
  get errorTitle() {
    return (this.searchText
      ...
      : `No data categories have been created yet`)
  }
