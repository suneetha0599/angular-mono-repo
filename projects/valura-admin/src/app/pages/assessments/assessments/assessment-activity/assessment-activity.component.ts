...
import { ACTIVITY, FIRST_PAGE, PAGE_SIZE } from '../constants';
import { ActivityLogUI } from '@admin-page/task-management/constant';
...
import { ActivityLogComponent } from '@admin-page/task-management/activity-log/activity-log.component';

@Component({
  selector: 'app-assessment-activity',
  imports: [ActivityLogComponent,],
  templateUrl: './assessment-activity.component.html',
  styleUrl: './assessment-activity.component.scss'
})
export class AssessmentActivityComponent {
  ...
  async getAuditLogs(pageNo: number = FIRST_PAGE): Promise<void> {
    ...
    pageNo === FIRST_PAGE ? (this.initialLoading = true) : (this.paginationLoading = true);
    this.requestLoading = true;
    const module = AUDIT_LOG_MODULE.ASSESSMENT;
    const entityType = this.isVendorContext ? AUDIT_LOG_ENTITY_TYPE.VENDOR_ASSESSMENT : AUDIT_LOG_ENTITY_TYPE.ASSESSMENT;

    const res = await this.apiHelperService.getAuditLogs(module, entityType, this.assessmentId, params);
    ...
    if (res?.items) {
      ...
      this.activityLogs = pageNo === FIRST_PAGE ? mappedLogs : [...this.activityLogs, ...mappedLogs];
    }
